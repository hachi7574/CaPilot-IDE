...
- **不要**在 profile 里 overlay `mcp_servers.*`：Codex 对该表是整表替换不是深合并。只写 `enabled = false`、没有 `command`/`url` 会变成 invalid transport，TUI 的 `config/batchWrite`（项目 trust）失败。`node_repl` 过期路径要改用户自己的 `~/.codex/config.toml`。
...
- 模型：**settings.yaml 探测**（provider-qualified，`dsh.rs` `model_catalog_probe`）——顺序对齐 TUI `listModels`（`llm-pi-ai` 先注册 → pi 路由在前，`llm-deepseek` 后挂 → `deepseek-official` flash/pro 在后）；`agent-default-model` 标默认（实测 `opencode-go/deepseek-v4-flash`）。Composer 按 catalog 下标对 picker 发方向键，顺序不一致会选错行。
...
- 换模型：**不杀 PTY**。运行中会话驱动 TUI 内置 `/model` 选择器（进程内 fork 续聊：历史保留、新 provider session id、同一 CaPilot tab）；随后 `agent_refresh_resume_key` 把 DB `resume_key` 钉到子会话。休眠会话只 `agent_set_session_config`，下次打开经 `--patch` 生效（`Composer.tsx` `applyModel` dsh 分支）。
...
| 23 | dsh 模型换法 | **不杀 PTY**。运行中：Composer 驱动 TUI `/model` 选择器（进程内 fork）+ `agent_refresh_resume_key`；休眠：只持久化，下次 spawn/`--patch` | `Composer.tsx` `applyModel` dsh 分支 + `agent_refresh_resume_key` + `dsh.rs` `write_patch` | dsh-tui `channel.switchModel` / ModelPicker |
