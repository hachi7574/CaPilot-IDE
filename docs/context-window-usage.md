...
Cache-hit accounting was removed (structured-agent-runtime-architecture §12.1): `AgentUsage` no longer carries cache hit rate fields, and the composer no longer renders a cache chip. Account-level quota stays in the independent provider quota service and is never mixed into this session sample.
